...
function Requests_run(method, url, headers, data, timeout, retryCount, verify, parseJson, parseJsonFlags)
    ...
    m.logger.verbose("[http] ------ START HTTP REQUEST ------", m.name)
    m.logger.verbose("[http] URL", m.name, { url: urlTransfer.GetURL() })
    m.logger.verbose("[http] Timeout", m.name, { timeout: timeout })
    m.logger.verbose("[http] Headers", m.name, { headers: headers })
    ...
    while retryCount >= 0
        ...
        m.logger.verbose("[http] Method", m.name, { method: method })
        if method = "POST"
        ...
        if sent = true
            ...
            if type(event) = "roUrlEvent"
                ...
                m.logger.verbose("[http] Response Code", m.name, { responseCode: responseCode })
                if responseCode > 0 AND responseCode < 400
                    'Response was good, so we break the while
                    exit while
                else
                    'We have a bad response
                    m.logger.verbose("[http] Bad response", m.name, { responseCode: responseCode })
                    m.logger.verbose("[http] Will Retry", m.name, { retryCount: retryCount })
                end if
            else
                if m.cancel_and_return = true
                    m.logger.verbose("[http] Killing the Task", m.name)
                    exit while
                else
                    'We timed out so we should cancel the request
                    m.logger.verbose("[http] Event Timed Out", m.name)
                    urlTransfer.AsyncCancel()
                    'Exponential backoff timeouts
                    timeout = timeout * 2
                    m.logger.verbose("[http] Timeout", m.name, timeout)
                end if
            end if
        end if
    end while
    m.logger.verbose("[http] ------ END HTTP REQUEST ------", m.name)
